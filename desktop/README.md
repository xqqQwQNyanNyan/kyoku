...
窗口默认内容尺寸为 1100×640，可拖动边缘缩小至 1000×560（逻辑像素，不含标题栏）。
启动时按屏幕可用区域限制尺寸，并为标题栏留出余量，避免初始窗口延伸到 Dock 下方。
小窗口内牌桌和复盘侧栏各自滚动，回放控制固定在底部。

导入本地天凤 JSON（最大 16 MiB），选择复盘玩家，即可按事件播放、拖动进度或按局跳转。
左右方向键前后移动，空格播放或暂停；输入框中的按键不控制回放。
事件编号表示该事件应用后的局面，与现有 CLI 一致。

牌桌随可用空间放大，分析与问答位于右侧。矮窗口下适当缩小牌桌、压缩问答区留白，
回放控制始终保留在窗口底部；牌桌空间不足时单独滚动浏览。
四家的手牌、副露和牌河分别定位，长牌河在预留区域内缩放。
